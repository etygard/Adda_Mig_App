...
class FileStorage {
    ...
    class func uploadImage (_ image: UIImage, directory: String, completion: @escaping (_ document: String?) -> Void) {
        let storageRef = storage.reference(forURL: kFILEREFFERENCE).child(directory)
        let imageData = image.jpegData(compressionQuality: 0.6)
        
        var task: StorageUploadTask!
        task = storageRef.putData(imageData!, metadata: nil, completion: { (metaDatra, error) in
    ...
    class func uploadImages (_ images: [UIImage?], completion: @escaping (_ imageLinks: [String]) -> Void) {
        ...
        for image in images {
            ...
            let fileDirectory = "UserImages/" + userModel.currentId() + "/" + "\(nameSuffix)" + ".jpg"
        ...
        if fileExistsAtPath(path: imageFileName) {
            ...
            if let contentOfFile = UIImage(contentsOfFile: fileInDocumentDirectory(filename: imageFileName)) {
              completion(contentOfFile)
                ...
                print("Could'n generate image from local image")
                completion(nil)
            }
        } else {
            if imageUrl != "" {
                ...
                downloadQueue.async {
                    ...
                    if data != nil {
                        ...
                        // save to locally for the next time when we open the app
        ...
        for link in imageUrls {
            ...
            downloadQueue.async {
                ...
                if data != nil {
                    ...
                    if downlaodCounter == imageArray.count {
    ...
    class func saveImageLocally(imageData: NSData, fileName: String) {
        ...
        imageData.write(to: docURL, atomically: true) //true  write a new file / overwrite or not
    }
}
...
func fileExistsAtPath(path: String) -> Bool {
     // the simple way to write, you will get the same result
     var doesExist = false
